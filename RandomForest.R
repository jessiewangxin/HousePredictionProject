...
rf.default = randomForest(LogSalePrice ~., data = data.train, importance = TRUE, ntree = 500, mtry=6)
...
data_rf_test = data_rf_test %>% filter(YearBuilt==YearRemodAdd)
data_rf_test = data_test%>% mutate_if(is.character, as.factor)
